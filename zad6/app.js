...
app.post('/kontakt', (req, res) => {
    ...
    const query = 'INSERT INTO messages (imie, nazwisko, email, wiadomosc) VALUES (?, ?, ?, ?)';
    db.query(query, [imie, nazwisko, email, wiadomosc]);
...
app.get('/api/contact-messages', (req, res) => {
    db.query('SELECT * FROM messages', (err, results) => {
        if (err) {
            return res.status(404).send('Bład 404');
        }
        res.json(results);
    });
});
...
app.get('/api/contact-messages/:id', (req, res) => {
    ...
    db.query('SELECT * FROM messages WHERE id = ?', [id], (err, results) => {
        if (err) {
            return res.status(404).send('Błąd 404');
        }
        res.json(results[0]);
    });
});
